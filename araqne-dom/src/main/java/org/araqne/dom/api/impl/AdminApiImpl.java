...
import org.araqne.dom.api.LoginCallback;
...
public class AdminApiImpl implements AdminApi {
				...
				if (maxSession > 0) {
					if (force) {
						while (loggedIn.size() >= maxSession) {
							...
							pushApi.push(ban.session, "araqne-system-event", m);
	...
	private void enforcePasswordChange(String domain, Admin admin) {
		logger.trace("araqne dom: last password change [{}]", admin.getUser().getLastPasswordChange());
		long interval = 0;
		...
		logger.trace("araqne dom: checking expired password interval [{}], last change [{}]", interval, baseline);
	...
	public void logout(Session session) {
		if (session.getOrgDomain() != null && session.getAdminLoginName() != null) {
			logger.trace("araqne dom: logout [domain: {}, login: {}]", session.getOrgDomain(), session.getAdminLoginName());
