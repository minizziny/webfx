...
import org.araqne.dom.api.LocalizationApi;
import org.araqne.dom.api.ResourceKey;
...
public class LocalizationApiImpl implements LocalizationApi, BundleListener {
	...
	public void bundleChanged(BundleEvent event) {
		if (event.getType() == BundleEvent.STARTED) {
			...
			for (ResourceKey key : m.keySet()) {
				...
				logger.trace("araqne dom: added [{}] resource key", key);
			}
			...
			for (ResourceKey key : m.keySet()) {
				templates.remove(key);
				logger.trace("araqne dom: removed [{}] resource key", key);
			}
		}
	}
	...
	private Map<ResourceKey, String> getLocalizations(BundleEvent event) {
		...
		Enumeration<String> e = bundle.getEntryPaths("/OSGI-INF/araqne-dom/");
		if (e == null) {
			logger.trace("araqne dom: localization not found for bundle {}", event.getBundle().getBundleId());
			return null;
		}

		while (e.hasMoreElements()) {
			String path = e.nextElement();
			if (path.startsWith("OSGI-INF/araqne-dom/localization.") && path.endsWith(".properties")) {
				String token = path.replace("OSGI-INF/araqne-dom/localization.", "").replace(".properties", "");
				if (token.length() != 2)
					continue;
				...
				try {
					...
					logger.error("araqne dom: cannot read localization properties", ex);
				} finally {
					if (reader != null)
						try {
							reader.close();
