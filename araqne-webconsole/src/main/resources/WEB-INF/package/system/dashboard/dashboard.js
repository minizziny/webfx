define(["/lib/jquery.js", "kradar.ui.js"], function(_$, ui) {
...
(function() {
	var resizable = kradar.namespace("ui.resizable");
	...
	kradar.ui.resizable = Resizable;

	kradar.ui.resizable.create = function(prop) {
		return new Resizable(prop);
	}
})();

(function() {
	var _box = kradar.namespace("ui.layout.box");
	...
	function Row(prop) {
		...
		this.deleteBox = function(box) {
			...
			var allidx = kradar.ui.layout.box.allboxes.indexOf(box);
			kradar.ui.layout.box.allboxes.removeAt(allidx);
			...
			if(box.row.boxes.length === 0) {
				...
				kradar.ui.layout.box.root.updateLayout(false);
			}
		}

		draw();

		var _super = kradar.extend(Row, kradar.ui.resizable, {
			"el": el,
	...
	function Box(prop) {
		...
		this.findBox = function(guid) {
			...
			var box, i = 0;
			var allboxes = kradar.ui.layout.box.allboxes;
		...
		this.appendTo = function(row_or_el) {
			...
			if(row_or_el.constructor.name === "Resizable" || row_or_el.constructor.name === "Row") {
				...
				kradar.ui.layout.box.root = that;
...
//								console.log(box.row.box.rows)
		...
		function draw() {
			...
			if(prop.rows == undefined) {
				var closebtn = $("<button>").addClass("btn").addClass("btn-mini")
											.text("x")
											.css("margin", "2px")
											.on("click",function() {
												that.close();
		...
		function activateDroppable(box) {
			$.each(kradar.ui.layout.box.allboxes, function(i, b) {
				if(!!b.droppable) {
					if(box != b) {
						b.droppable.show();
		...
		function deactivateDroppable() {
			$.each(kradar.ui.layout.box.allboxes, function(i, box) {
				if(!!box.droppable) {
					box.droppable.hide();
				}
			});
		...
		function drop(box) {
			...
			if(!!kradar.ui.layout.box.target) {
				var t = kradar.ui.layout.box.target;
				var target = t.splitTarget;
				delete t.splsplitTarget;
				
				t.splitInsert(box, target);
				
				kradar.ui.layout.box.target = null;
			}
			else {
				box.el.css("position", "")
					  .css("top", "")
		...
		function makeDroppable(box, isContainer) {
			...
			box.dropCompass.on("mouseover", function(e) {
				...
				kradar.ui.layout.box.target = box;
			})
			.on("mouseout", function() {
				$(this).removeClass("over1");
				delete box.splitTarget;
				kradar.ui.layout.box.target = undefined;
		...
		function unwrapRow(row) {
			...
			var allidx = kradar.ui.layout.box.allboxes.indexOf(row.box);
			kradar.ui.layout.box.allboxes.removeAt(allidx);
		...
		var _super = kradar.extend(Box, kradar.ui.resizable, {
			"el": el,
		...
		kradar.ui.layout.box.allboxes.push(this);
	}

	_box.create = function(prop, isreset) {
		if(isreset != undefined) {
			if(isreset) {
				kradar.ui.layout.box.allboxes = [];
			}
		}
		return new Box(prop);
	}
