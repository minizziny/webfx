...
public class KeyDistributorServlet extends HttpServlet {
	...
	public void log(String message, Throwable t) {
		logger.warn("araqne webconsole: key dist error", t);
	}
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (session == null) {
			logger.warn("araqne webconsole: cannot issue enc key, unknown session {}", id);
			return;
		}

		logger.trace("araqne webconsole: session [{}] new enc key [{}]", session.getGuid(), key);
