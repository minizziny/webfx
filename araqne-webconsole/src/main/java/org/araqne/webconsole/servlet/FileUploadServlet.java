...
import org.araqne.httpd.MimeTypes;
...
public class FileUploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException {
		...
		logger.trace("araqne webconsole: jsession [{}]", req.getSession().getId());
		...
		try {
			...
			logger.trace("araqne webconsole: open downstream for {}", f.getFile().getAbsolutePath());
			...
			resp.setStatus(500);
			logger.warn("araqne webconsole: cannot download id " + fileGuid, e);
		} finally {
			if (is != null)
				try {
					is.close();
	...
	private String getDownloadToken(HttpServletRequest req) {
		...
		if (cookies != null) {
			for (Cookie cookie : cookies) {
				logger.trace("araqne webconsole: checking all cookie for download, {} = {}", cookie.getName(),
						cookie.getValue());
				if (cookie.getName().equals("araqne_session"))
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException {
		logger.debug("araqne webconsole: received post request from [{}]", req.getRemoteAddr());

		String token = req.getHeader("X-Upload-Token");
		if (token == null) {
			logger.warn("araqne webconsole: upload token header not found for [{}] stream", req.getRemoteAddr());
			return;
		}
		...
		try {
			...
			if (range == null) {
				...
				logger.info("araqne webconsole: partial upload, token [{}], [{}~{}]",
			...
			logger.warn("araqne webconsole: upload post failed", e);
		} finally {
			if (is == null)
				return;
