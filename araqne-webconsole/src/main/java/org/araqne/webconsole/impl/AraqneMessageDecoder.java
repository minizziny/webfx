...
public class AraqneMessageDecoder {
	private AraqneMessageDecoder() {
	}

	public static Message decode(Session session, String text) {
		Logger logger = LoggerFactory.getLogger(AraqneMessageDecoder.class.getName());
		...
		if (logger.isDebugEnabled())
			logger.debug("araqne webconsole: debug websocket frame length {}, json [{}]", text.length(), text);
		...
		if (session.has("enc_key")) {
			try {
				...
				if (header.containsKey("iv") && body.containsKey("data")) {
					...
					logger.trace("araqne webconsole: decrypted msg [{}]", text);
				}
			} catch (Exception e) {
				logger.error("araqne webconsole: cannot decode encrypted msg [" + text + "]", e);
			}
		}
		...
		try {
			...
			logger.error("araqne webconsoke: invalid json Processing", e);
		} catch (IOException e) {
			logger.error("araqne webconsoke: invalid json read", e);
		}
		return null;
	}
