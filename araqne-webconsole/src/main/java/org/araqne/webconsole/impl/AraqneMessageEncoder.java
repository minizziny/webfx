...
public class AraqneMessageEncoder {
	private static Logger logger = LoggerFactory.getLogger(AraqneMessageEncoder.class);

	private AraqneMessageEncoder() {
	}
	...
	public static String encode(Session session, Message msg) {
		...
		if (session.has("enc_key")) {
			try {
				...
				logger.error("araqne webconsole: cannot encrypt msg", e);
			}
		}
	...
	private static String jsonize(Map<String, Object> headers, Map<String, Object> properties) {
		...
		try {
			...
			logger.error("araqne webconsole: json encode error", e);
		}
